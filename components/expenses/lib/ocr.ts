...
const compareItemOCRValue = (
  ...
  const checkValue = ocrValue => {
    if (isNil(existingValue) || isNil(ocrValue)) {
      ...
      return {
        hasMismatch: ocrValue.currency !== form.values.currency || existingValue !== ocrValue.valueInCents,
        ocrValue,
      };
    } else {
...
type OCRValuesComparison = Record<FieldsWithOCRSupport, { hasMismatch: boolean; ocrValue: any }>;
